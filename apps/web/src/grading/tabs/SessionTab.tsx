...
import { latestByAnswer } from "../decisions";
...
/** Latest decision per answer, sorted by student index. */
function latest(decisions: Decision[]) {
  return [...latestByAnswer(decisions).values()].sort((a, b) => a.studentIndex - b.studentIndex);
}

export function SessionTab({ decisions, alertsRaised, alertsAligned, checksRaised, checksApproved, totalStudents, maxPoints }: Props) {
  ...
  return (
    <>
      <div className="gg-stats">
        <div className="gg-stat"><div className="gg-n" data-gg-stat-graded>{all.length}</div><div className="gg-l">graded here</div></div>
      ...
      {all.length === 0 ? (
        <div className="gg-status">Grade a few students on this question and the comparison chart will appear here.</div>
      ) : (
        <>
          <svg className="gg-chart" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Your grade and the rubric-referenced grade per student" data-gg-chart>
            ...
            {withSuggestion.length > 1 && (
              <polyline fill="none" stroke="#8a949c" strokeWidth={1.4} strokeDasharray="4 3" points={withSuggestion.map((d) => `${x(d.studentIndex)},${y(d.suggestedPoints as number)}`).join(" ")} data-gg-suggested-line />
            )}
            ...
            {all.length > 1 && <polyline fill="none" stroke="#5b3df5" strokeWidth={1.8} points={all.map((d) => `${x(d.studentIndex)},${y(d.points)}`).join(" ")} data-gg-grade-line />}
            {all.map((d) => (
              <circle key={d.id} cx={x(d.studentIndex)} cy={y(d.points)} r={3} fill="#5b3df5" data-gg-point />
            ))}
          </svg>
